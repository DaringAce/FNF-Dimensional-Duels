...
//		['F', 0.5], //From 40% to 49%
	...
	public var dangerLevel:Float = 0; //Danger Level number
	...
	//Ace's week BG character
	var AcesHouseBGLuna:Character; 

	//Camera Anim Offset
	var camMoveShit:Array<Float>; //dad x, dad y, bf x, bf y
    var camMoveAmount:Float = 50;
    var camZoomerAllow:Bool = true;

	public var songScore:Int = 0;
	public var songHits:Int = 0;
	public var songMisses:Int = 0;
	public var scoreTxt:FlxText;
	public var dangerLevelTxt:FlxText; //Danger Level TEXT
	var timeTxt:FlxText;
	...
	var iconP1AngleTween:FlxTween; //Health icon bop, Player 1
	var iconP2AngleTween:FlxTween; //Health icon bop, Player 2
	var aceTrailArea:FlxTrailArea; //Ace Afterimage
	var ace:Character;
	...
	var aceGOD:Bool = false;
	public static var pumpkins:Int = 0;
	private var ss:Bool = false;
	
	//Week 2
	public var pumpkinbarOUTLINE:FlxSprite; // Pumpkin Mechanic Bar
	public var aceDrainRate:Float = 0.0333; // Ace's Health Drain Amount
	
	//Week 3 Mechanic Bar
	public var firebarOUTLINE:FlxSprite;
	...
	override public function create()
	{
		...
		if(SONG.stage == null || SONG.stage.length < 1) {
			switch (songName)
			{
				...
				case 'ugh' | 'guns' | 'stress':
					curStage = 'tank';
				case 'long-wired-brawl' | 'Long-Wired Brawl':
					curStage = 'AcesHouse';
				case 'Scythe' | 'Righteous-Waterstream' | 'Righteous Waterstream' | 'Grand-Finale' | 'Grand Finale':
		...
		switch (curStage)
		{
			...
			case 'AcesHouse':
			{						
				var house = Paths.getSparrowAtlas('AcesHouse','Duel1');
			...
			case 'AcesHouseEvil':
			{				
				var house = Paths.getSparrowAtlas('AcesHouseEVIL','Duel1');
...
//		timeTxt.setFormat(Paths.font("PhantomMuff.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		timeTxt.setFormat(Paths.font("PhantomMuff.ttf"), 32, FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]), CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
...
//		add(songTitle);
		...
		// <----- DANGER LEVEL TEXT ----->
		dangerLevelTxt = new FlxText(timeTxt.x + (timeTxt.x * 2.0), timeTxt.y - 4, FlxG.width, "DAMAGED: " + dangerLevel + "%", 26);
		dangerLevelTxt.setFormat(Paths.font("OBELIX PRO BROKEN.ttf"), 26, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		dangerLevelTxt.scrollFactor.set();
		dangerLevelTxt.borderSize = 1;
		dangerLevelTxt.visible = true;
//		add(dangerLevelTxt);
		if(ClientPrefs.downScroll) {
			dangerLevelTxt.y = timeTxt.y;
		}
		// ^ ----- END ----- ^

		botplayTxt = new FlxText(0, timeBarBG.y + 60, FlxG.width, "<---- AUTOPLAY ---->", 34);
		...
		add(botplayTxt);
		if(ClientPrefs.downScroll) {
			botplayTxt.y = timeBarBG.y - 85;
		}
		if(!ClientPrefs.middleScroll){
			botplayTxt.x = STRUM_X + (FlxG.width / 4) - 46;
		}
		...
		dangerLevelTxt.cameras = [camHUD]; //Danger Level Text: Camera Setting
...
//		var foldersToCheck:Array<String> = [Paths.mods('shaders/')];
		var foldersToCheck:Array<String> = [Paths.mods('shaders/'), 'assets/shaders/'];
		if(Paths.currentModDirectory != null && Paths.currentModDirectory.length > 0)
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				var introAssets:Map<String, Array<String>> = new Map<String, Array<String>>();
				introAssets.set('default', ['countdown-are', 'countdown-you', 'countdown-ready']);
				introAssets.set('pixel', ['pixelUI/ready-pixel', 'pixelUI/set-pixel', 'pixelUI/date-pixel']);
				...
				{
/*					case 0:
						FlxG.sound.play(Paths.sound('' + introSoundsSuffix), 0.6);
*/					case 1:
						...
						FlxG.sound.play(Paths.sound('duelistCD3' + introSoundsSuffix), 0.6);
					case 2:
						...
						FlxG.sound.play(Paths.sound('duelistCD2' + introSoundsSuffix), 0.6);
					case 3:
						...
						FlxG.sound.play(Paths.sound('duelistCD1' + introSoundsSuffix), 0.6);
					case 4:
				}
...
//			case 'Xonric Outline Effect':
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Xonric Outline Effect':
				trace('Xonric Outline Event triggered...');
	...
	function camFollowPosFunk(value1:Float, value2:Float){
	...
	function offsetCamMove()
	{
		if(startedCountdown && curStep > 0 && !inCutscene && camZoomerAllow && !endingSong){
                if (SONG.notes[Math.floor(curStep/16)].mustHitSection) {
                    switch(boyfriend.animation.curAnim.name){
                        case 'singLEFT' | 'singLEFT-alt':
                            camFollowPosFunk(camMoveShit[2] - camMoveAmount, camMoveShit[3]);
                        case 'singRIGHT' | 'singRIGHT-alt':
                            camFollowPosFunk(camMoveShit[2] + camMoveAmount, camMoveShit[3]);
                        case 'singDOWN' | 'singDOWN-alt':
                            camFollowPosFunk(camMoveShit[2], camMoveShit[3] + camMoveAmount);
                        case 'singUP' | 'singUP-alt':
                            camFollowPosFunk(camMoveShit[2], camMoveShit[3] - camMoveAmount);
                        default:
                            camFollowPosFunk(camMoveShit[2], camMoveShit[3]);
                    }
                } else {
                    switch(dad.animation.curAnim.name){
                        case 'singLEFT' | 'singLEFT-alt':
                            camFollowPosFunk(camMoveShit[0] - camMoveAmount, camMoveShit[1]);
                        case 'singRIGHT' | 'singRIGHT-alt':
                            camFollowPosFunk(camMoveShit[0] + camMoveAmount, camMoveShit[1]);
                        case 'singDOWN' | 'singDOWN-alt':
                            camFollowPosFunk(camMoveShit[0], camMoveShit[1] + camMoveAmount);
                        case 'singUP' | 'singUP-alt':
                            camFollowPosFunk(camMoveShit[0], camMoveShit[1] - camMoveAmount);
                        default:
                            camFollowPosFunk(camMoveShit[0], camMoveShit[1]);
                    }
                }
            }
	}
	...
	function tweenCamIn() {
		if (Paths.formatToSongPath(SONG.song) == 'tutorial' && cameraTwn == null && FlxG.camera.zoom != 1.3) {
			cameraTwn = FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom}, (Conductor.stepCrochet * 4 / 2000), {ease: FlxEase.elasticInOut, onComplete:
				function (twn:FlxTween) {
					cameraTwn = null;
				}
			});
...
/*
			if (dad.curCharacter.startsWith('Ace'))
			{
				...
				switch(animToPlay)
				{
					case 'singLEFT':
					{
						aceTrailArea.setSize(dad.width + (dad.width * 0.2), dad.height + (dad.height * 0.05));
						aceTrailArea.x = dad.x - 10;
						aceTrailArea.y = dad.y;
					}
					case 'singRIGHT': 
					{
						aceTrailArea.setSize(dad.width + (dad.width * 0.2), dad.height + (dad.height * 0.05));
					...
					case 'singUP':
					{
						aceTrailArea.setSize(dad.width + (dad.width * 0.05), dad.height + (dad.height * 0.2));
						aceTrailArea.x = dad.x;
						aceTrailArea.y = dad.y;
					}
					case 'singDOWN':
					{
						aceTrailArea.setSize(dad.width + (dad.width * 0.25), dad.height);
						aceTrailArea.x = dad.x;
						aceTrailArea.y = dad.y;
					}
					default:
...
*/
	...
	function updateDanger(damageTaken:Float):Void
	{
		if(dangerLevel + damageTaken >= 100.0) //Instant Kill, if Danger Level is greater than or equal to 100%
		{
			health = 0; //Die

			printDebugText("YOUR Danger Level: " + (dangerLevel + damageTaken) + "%\nMAX Danger Level: 100.0%"); //Danger Level debug print
		}else //If Danger Level is still less than 100%
		{
			dangerLevel = Math.floor(dangerLevel + damageTaken); //Update Danger Level number
			dangerLevelTxt.text = "DAMAGED: " + dangerLevel + "%"; //Update Danger Level text
		}
	}

	function printDebugText(toPrint:String):Void //Test printer for debugging
	{
		trace("\n\n<----------------- !DEBUG PRINT! ----------------->\n <MSG>: " + toPrint + "\n<------------------------------------------------->\n\n");
	}

	function pumpkinAttack():Void //Pumpkin note mechanic -> Ace's week
	{
		var pumpkinEFF = new Emeraldspr(585, 325);
		pumpkinEFF2 = new Emeraldspr( -770, -450);
		
		if (pumpkins < 6)
		{
			pumpkins += 1;
			updateDanger(100 / 6); //Pumpkin damage
		}
		
		FlxG.sound.play(Paths.sound('thunder_1'));
		pumpkinEFF.animation.addByPrefix('Hit', 'Fire', 20, true);
		
		add(pumpkinEFF);
		pumpkinEFF.animation.play('Hit', true);
		
		gf.playAnim('scared', true);
		
		new FlxTimer().start(1.15, function(tmr:FlxTimer)
			{
				FlxTween.tween(pumpkinEFF, {y: pumpkinEFF.y += 0, alpha: 0}, Conductor.crochet / 1000, {
						ease: FlxEase.cubeInOut,
						onComplete: function(twn:FlxTween)
						{
							pumpkinEFF.destroy();
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		...
		if(dad.curCharacter == 'ace' && note.mustPress == false) skin = 'aceNoteAssets';
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'righteous-waterstream')
		{
